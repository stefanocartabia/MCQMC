...
function Probit_Gibbs_sampler(model::Probit_Gibbs_WCUD, seq::Array{Float64}, n_iter::Int, burn_in::Int, R::Int, alpha_level::Float64=0.95)
    ...
    for r in 1:R
        ...
        Beta_0 =  (X' * X) \ (X' * Y)                   # length p 
        z = rand.(Normal.(X * Beta_0, 1.0))             # length n (number of latent variables)
        Betas = copy(Beta_0)
