...
function UbTau(m, seq, mdl, k, f)
    p = mdl.p                                    
    x_t = Matrix{Float64}(undef, p+1, m+1)        
    y_t = Matrix{Float64}(undef, p+1, m)          
...
function R_UbTau(R::Int, n_iter::Int, seq, mdl, k, f)
    tau_vals = Vector{Int}(undef, R)

    for r in 1:R
        tau_vals[r] = UbTau(n_iter, seq[:,:,r], mdl, k, f)
    end
