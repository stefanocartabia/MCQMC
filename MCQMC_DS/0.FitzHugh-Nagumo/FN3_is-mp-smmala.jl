...
#########################################               FitzHugh–Nagumo (alpha, beta, gamma)                ###################################
###############################################################################################################################################
...
function IS_MP_sMALA(f!::Function, u0, obs, d::Integer, K::Integer, cov_mat, tspan, dt, priors, init_par; seq::AbstractMatrix{<:Float64}, N_prop::Integer, N_iter::Integer, step_size::Float64)
    ...
    grad_save = Array{Float64}(undef, N_iter, D)
    accept_proxy = 0.0
    ...
    for l in 1:N_iter
        ...
        # Monitor gradient
        grad_save[l,:] = grad!(grad, f_logpost, xI, cfg_1)
...
#----------------------------------------------------------- Orbits  ---------------------------------------------------------------------------------#
p1 = plot(sol[1,:], sol[2,:], linewidth = 1.5, title = "True FitzHugh–Nagumo Orbit", 
          xlabel = "x", ylabel = "y", legend = false, grid = true, gridalpha = 0.3 )
p2 = plot(obs_noisy[1,:], obs_noisy[2,:], linewidth = 1.5, title = "Observed FitzHugh–Nagumo Orbit", 
          xlabel = "x", ylabel = "y", legend = false, color = "red", grid = true, gridalpha = 0.3)
plot_traj = plot( p1, p2, layout = (1, 2), size = (1100, 400), top_margin = 3mm)
savefig(plot_traj, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_trajectory.png")

#----------------------------------------------------------- Time Series  ----------------------------------------------------------------------------#
FitzHugh_Nagumo_ts = plot( sol.t, obs_noisy[1,:], seriestype = :scatter, markersize = 3, alpha = 0.5, label = "x obs", 
                           color = :lightblue, grid = true, gridalpha = 0.3, xlabel = "t", ylabel = "states", legend = :outertop, )
plot!( sol.t, sol[1,:], lw = 2, label = "x true", color = :blue, )
plot!( sol.t, obs_noisy[2,:], seriestype = :scatter, markersize = 3, alpha = 0.5, label = "y obs", color = :lightcoral, )
plot!( sol.t, sol[2,:], lw = 2, label = "y true", color = :red, )
plot!( title = "\n FitzHugh–Nagumo System Dynamics", size = (1000, 800), left_margin = 5mm, bottom_margin = 5mm, 
       top_margin = 5mm, legendcolumns = 2, legendborder = false, legend_foreground_color = :transparent, )

savefig(FitzHugh_Nagumo_ts, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_trajectory_ts.png")
...
for eps in step_size
    tuning_time = @elapsed pre_run = IS_MP_sMALA(fhn!, u0, obs_noisy, size(obs_noisy, 1), size(obs_noisy, 2), 
...
savefig(plot_weight_start, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_tuning_eps.png")

for eps in step_size
    fname = "C:\\Users\\mussi\\Documents\\Manhattan\\Leuven\\MCQMC\\Plots\\is-mp-smmala_ad\\FHM\\weights_start_eps_$(replace(string(eps), "." => "_")).png"
    isfile(fname) && rm(fname)
end

############################################################## Run IS-MP-smMALA FitzHugh–Nagumo System ########################################################################
...
savefig(p_weights_evolution, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_weights.png")
#-------------------------------------------------------------------------------------------------------------# 
#  Trace Plots for parameters
chain = exp.(out.chain)
iters = 1:size(chain, 1)

p1 = plot(iters, chain[:, 1], label="", title="Trace of α", xlabel="Iteration", ylabel="Value")
hline!(p1, [par_true[1]], linestyle=:dash, color=:red)
p2 = plot(iters, chain[:, 2], label="", title="Trace of β", xlabel="Iteration", ylabel="Value")
hline!(p2, [par_true[2]], linestyle=:dash, color=:red)
p3 = plot(iters, chain[:, 3], label="", title="Trace of γ", xlabel="Iteration", ylabel="Value")
hline!(p3, [par_true[3]], linestyle=:dash, color=:red)
p = plot(p1, p2, p3, layout=(3,1), size=(900,800), legend=false)
savefig(p, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_TracePlots.png")
...
p1 = plot(iters, chain[:, 1], label="", title="Gradient Trace of α", xlabel="Iteration", ylabel="Value")
hline!(p1, [0], linestyle=:dash, color=:red)
p2 = plot(iters, chain[:, 2], label="", title="Gradient Trace of β", xlabel="Iteration", ylabel="Value")
hline!(p2, [0], linestyle=:dash, color=:red)
p3 = plot(iters, chain[:, 3], label="", title="Gradient Trace of γ", xlabel="Iteration", ylabel="Value")
hline!(p3, [0], linestyle=:dash, color=:red)
p = plot(p1, p2, p3, layout=(3,1), size=(900,800), legend=false)
savefig(p, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_GradientTracePlots.png")

####--------------------------------------------------------------------------------------------####
# Reconstruct trajectories with estimated parameters
fhn_new = ODEProblem(fhn!, u0, tspan, tuple(par_est...))
sol_new = solve(fhn_new, Tsit5(), saveat = dt)

p_overlay = plot(sol[1, :], sol[2, :], linewidth = 1.5, color = "blue", label = "True orbit",
                 xlabel = "x", ylabel = "y", zlabel = "z", title = "FitzHugh–Nagumo Orbits (Overlayed)", grid = true, gridalpha = 0.3, fontsize = 8)
scatter!(p_overlay, obs_noisy[1, :], obs_noisy[2, :], markersize = 2, color = "red", alpha = 0.3, label = "Observations")
plot!(p_overlay, sol_new[1, :], sol_new[2, :], linewidth = 1.5, color = "green", label = "Reconstructed orbit")

savefig(p_overlay, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_FitzHugh–Nagumo_Orbits.png")

####--------------------------------------------------------------------------------------------####
# Time series plot 
p1 = scatter(sol.t, obs_noisy[1,:], markersize=3, alpha=0.5, label="u Observed", color=:lightblue,
             grid=true, gridalpha=0.3, ylabel="u", legend=:outertop)
plot!(p1, sol.t, sol_new[1,:], lw=2, label="u Reconstructed", color=:blue)

p2 = scatter(sol.t, obs_noisy[2,:], markersize=3, alpha=0.5, label="v Observed", color=:lightcoral,
             grid=true, gridalpha=0.3, ylabel="v", legend=:outertop)
plot!(p2, sol.t, sol_new[2,:], lw=2, label="v Reconstructed", color=:red)

FitzHugh_Nagumo_ts = plot(p1, p2, layout=(2,1), size=(1000, 800), 
                plot_title="FitzHugh–Nagumo Dynamics (IS-MP-smMALA)", 
...
savefig(FitzHugh_Nagumo_ts, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_FitzHugh–Nagumo_TimeSeries.png")
...
function f_sse(p_test)
    ...
    lor_end = ODEProblem(fhn!, u0, tspan, p_test)
...
##-------------------------------------------- Marginal Likelihood for FitzHugh–Nagumo Parameters ---------------------------------------------------#

n_points = 5000

# Parameter ranges for alpha, beta, gamma
param_ranges = [
    range(0.0, 5.0, length=n_points),    # alpha
    range(0.0, 12.0, length=n_points),   # beta
    range(0.5, 20.0, length=n_points)    # gamma
]
...
for param_idx in 1:3
    for (i, param_val) in enumerate(param_ranges[param_idx])
        ...
        sim = simulate_system(fhn!, u0, exp.(theta_test), tspan, dt)
...
p1 = plot(param_ranges[1], loglik_matrix[:, 1], 
    linewidth=2, xlabel= "α", ylabel="Log-Likelihood", 
...
p3 = plot(param_ranges[3], loglik_matrix[:, 3], 
    linewidth=2, xlabel= "γ", ylabel="Log-Likelihood", 
...
p_profiles = plot(p1, p2, p3, layout=(1,3), size=(1500, 600), plot_title="Marginal Log-Likelihoods - FitzHugh–Nagumo", 
                  left_margin= 10mm, bottom_margin=10mm,legend=:outertop, legendcolumns=2, legendfontsize=11, 
                  legendborder=false, legend_foreground_color=:transparent)
display(p_profiles)

savefig(p_profiles, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_FitzHugh–Nagumo_Marginal_Likelihoods.png")

##-------------------------------------------------- Marginal Posterior for FitzHugh–Nagumo Parameters --------------------------------------------------------#
...
for param_idx in 1:3
    for (i, param_val) in enumerate(param_ranges[param_idx])
        ...
        sim = simulate_system(fhn!, u0, par_test, tspan, dt)
...
pp1 = plot(param_ranges[1], logpost_matrix[:, 1], 
    linewidth=2, xlabel="α", ylabel="Log-Posterior", 
    title="α", grid=true, gridalpha=0.3, label="")
...
pp3 = plot(param_ranges[3], logpost_matrix[:, 3], 
    linewidth=2, xlabel="γ", ylabel="Log-Posterior", 
    title="γ", grid=true, gridalpha=0.3, label="")
...
p_posteriors = plot(pp1, pp2, pp3, layout=(1,3), size=(1500, 600), 
    plot_title="Marginal Log-Posterior - FitzHugh–Nagumo", left_margin=6mm, bottom_margin=5mm,
...
savefig(p_posteriors, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\FHM\MC_FitzHugh–Nagumo_Marginal_Log-Posteriors.png")
