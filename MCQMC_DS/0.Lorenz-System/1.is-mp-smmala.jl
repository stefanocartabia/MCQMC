...
function simulate_system(f!::Function, u0, tspan::Tuple, dt)
...
# Log-posterior for initial conditions (NO Jacobian adjustment - u0 already in ℝ³)
function build_log_posterior(f!::Function, obs, save_chol_cov::Chol_Save, tspan, dt, logprior_u0::Function, d::Integer, K::Integer)
    
    function logpost(u0::AbstractVector)
        sim = simulate_system(f!, u0, tspan, dt)
        return loglik_gaussian(obs, sim, save_chol_cov, d, K) + logprior_u0(u0)
    end
    return logpost
end
...
# Riemannian Metric: Expected Fisher Information Matrix w.r.t. initial conditions
function Tensor_Metric_sm(f!, u0::AbstractVector{<:Real}, tspan, dt, d::Integer, K::Integer, save_chol_cov::Chol_Save, priors; λ::Float64 = 1e-6)

    D = length(u0)
    chol_cov = save_chol_cov.chol_cov

    function sim_vec(u0_local::AbstractVector)
        return vec(simulate_system(f!, u0_local, tspan, dt))
    end

    cfg = ForwardDiff.JacobianConfig(sim_vec, u0, ForwardDiff.Chunk{3}())
    J = ForwardDiff.jacobian(sim_vec, u0, cfg)   # (K*d) × D
    ...
    # Prior precision for Normal distributions
    prior_diag = similar(u0)
    for i in eachindex(u0)                  
        σ = std(priors[i])  # Normal(μ, σ)
        prior_diag[i] = 1 / (σ^2)
    end
...
function IS_MP_sMALA_Lorenz(f!::Function, obs::AbstractMatrix, d::Integer, K::Integer, cov_mat::AbstractMatrix, 
                           tspan::Tuple, dt::Real, priors::AbstractVector, init_u0::AbstractVector; 
                           seq::AbstractMatrix{<:Float64}, N_prop::Integer, N_iter::Integer, step_size::Float64=0.01)

    D = length(init_u0)
    @assert D == 3 "This implementation expects 3-dimensional initial conditions."
    ...
    # State and storage
    xI = init_u0                             # Initial point in state space                          
    I_new = 1
    ...
    # Log-prior function
    logprior_u0 = u -> (logpdf(priors[1], u[1]) + logpdf(priors[2], u[2]) + logpdf(priors[3], u[3]))

    # Definition Log-posterior distribution
    f_logpost = build_log_posterior(f!, obs, save, tspan, dt, logprior_u0, d, K)
    cfg_1 = ForwardDiff.GradientConfig(f_logpost, init_u0, ForwardDiff.Chunk{D}())
    ...
    for l in 1:N_iter
        ...
        proposals[1, :, l] = xI
        metric = Tensor_Metric_sm(f!, xI, tspan, dt, d, K, save, priors)
        ...
        metric_z = Tensor_Metric_sm(f!, z, tspan, dt, d, K, save, priors)
        ...
        for i in 1:(N_prop + 1)
            ...
            # μ(yi): Langevin drift 
            metric_y = Tensor_Metric_sm(f!, yi, tspan, dt, d, K, save, priors)
    ...
    return (chain_u0 = chain[1:l_effective,:],
            accept_proxy = accept_proxy / l_effective,     
            weights = w[1:l_effective,:],
            grad = grad,
            length = l_effective,
...
lor_sys = ODEProblem(lorenz!, u0, tspan, nothing);  # nothing for parameter-free in-place function
...
#----------------------------------------------------------- Orbits  ---------------------------------------------------------------------------------#
p1 = plot(sol[1,:], sol[2,:], sol[3,:], linewidth = 1.5, title = "Lorenz Attractor\n" * "(ρ=28, σ=10, β=8/3)", 
...
savefig(plot_traj, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorenz\MC_trajectory.png")
#-------------------------------------------------------------------------------------------------------------------------------------------------------#

μ0 = [4.0, 4.0, 4.0]; prior_std_x0 = 0.1
prior_x0 = MvNormal(μ0, Diagonal(fill(prior_std_x0^2, 3)))

# Priors as vector of Normal distributions
priors = [Normal(μ0[i], prior_std_x0) for i in 1:3]

N_prop = 6; N_iter = 4000
...
for eps in step_size
    tuning_time = @elapsed pre_run = IS_MP_sMALA_Lorenz(lorenz!, obs_noisy, size(obs_noisy, 1), size(obs_noisy, 2), sigma_eta,
                                                        tspan, dt, priors, μ0, seq=wcud, N_prop=N_prop, N_iter=N_iter, step_size=eps);
...
savefig(plot_weight_start, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorenz\MC_tuning_eps.png")

for eps in step_size
    fname = "C:\\Users\\mussi\\Documents\\Manhattan\\Leuven\\MCQMC\\Plots\\is-mp-smmala_ad\\Lorenz\\weights_start_eps_$(replace(string(eps), "." => "_")).png"
    isfile(fname) && rm(fname)
end

############################################################## Run IS-MP-smMALA Lorenz System ########################################################################
mcqmc_time = @elapsed out = IS_MP_sMALA_Lorenz(lorenz!, obs_noisy, size(obs_noisy, 1), size(obs_noisy, 2), sigma_eta,
                                                tspan, dt, priors, μ0, seq=wcud, N_prop=N_prop, N_iter=N_iter, step_size=0.2);

println("Execution time: $(mcqmc_time) sec")
println("Chain stops after $(out.length) of $(N_iter).")
out.chain_u0 
state_est = vec(mean(out.chain_u0[out.length÷2:end, :], dims=1))
...
savefig(p_weights_evolution, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorenz\MC_LS_weights.png")
#-------------------------------------------------------------------------------------------------------------# 
# Trace plots for initial conditions
chain = out.chain_u0
iters = 1:size(chain, 1)

p1 = plot(iters, chain[:, 1], label="", title="Trace of first coordinate", xlabel="Iteration", ylabel="Value")
hline!(p1, [u0[1]], linestyle=:dash, color=:red)
p2 = plot(iters, chain[:, 2], label="", title="Trace of second coordinate", xlabel="Iteration", ylabel="Value")
hline!(p2, [u0[2]], linestyle=:dash, color=:red)
p3 = plot(iters, chain[:, 3], label="", title="Trace of third coordinate", xlabel="Iteration", ylabel="Value")
hline!(p3, [u0[3]], linestyle=:dash, color=:red)
p = plot(p1, p2, p3, layout=(3,1), size=(900,800), legend=false)
savefig(p, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorentz\MC_LS_TracePlots.png")

####--------------------------------------------------------------------------------------------####
# Reconstruct trajectories with estimated initial conditions
lor_sys_new = ODEProblem(lorenz!, state_est, tspan, nothing)
...
savefig(p_overlay, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorentz\MC_Lorenz_Orbits.png")
...
lorenz_ts = plot(p1, p2, p3, layout=(3,1), size=(1000, 800), plot_title="Lorenz Dynamics (IS-MP-smMALA)", 
                left_margin=5mm, bottom_margin=5mm, top_margin=5mm, legend_foreground_color=:transparent,
                legend=:outertop, legendcolumns=2)

savefig(lorenz_ts, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorentz\MC_Lorenz_TimeSeries.png")

####--------------------------------------------------------------------------------------------####
# SSE function for initial conditions
function f_sse(u0_test)
    sse = 0.0
    resid = zeros(size(obs_noisy, 1))
    lor_end = ODEProblem(lorenz!, u0_test, tspan, nothing)
...
##-------------------------------------------- Marginal Likelihood for Lorenz Initial Conditions ---------------------------------------------------#

n_points = 2000

# Parameter ranges for initial conditions (u0)
param_ranges = [
    range(-1, 5,  length=n_points),  # x0
    range(-1, 5, length=n_points),  # y0
    range(-1, 5, length=n_points)   # z0
]

loglik_matrix = Array{Float64}(undef, n_points, 3)

u0_fixed = vec(state_est)  # Use estimated initial conditions

# Compute profile likelihood for each initial condition
for param_idx in 1:3
    for (i, param_val) in enumerate(param_ranges[param_idx])
        u0_test = copy(u0_fixed)
        u0_test[param_idx] = param_val
        sim = simulate_system(lorenz!, u0_test, tspan, dt)
...
p1 = plot(param_ranges[1], loglik_matrix[:, 1], 
    linewidth=2, xlabel= "x₀", ylabel="Log-Likelihood", 
    title="x₀", grid=true, gridalpha=0.3, label="")
vline!(p1, [u0[1]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(p1, [state_est[1]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

p2 = plot(param_ranges[2], loglik_matrix[:, 2], 
    linewidth=2, xlabel= "y₀", ylabel="Log-Likelihood", 
    title="y₀", grid=true, gridalpha=0.3, label="")
vline!(p2, [u0[2]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(p2, [state_est[2]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

p3 = plot(param_ranges[3], loglik_matrix[:, 3], 
    linewidth=2, xlabel= "z₀", ylabel="Log-Likelihood", 
    title="z₀", grid=true, gridalpha=0.3, label="")
vline!(p3, [u0[3]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(p3, [state_est[3]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

p_profiles = plot(p1, p2, p3, layout=(1,3), size=(1300, 800), plot_title="Marginal Log-Likelihoods - Lorenz Initial Conditions", 
...
savefig(p_profiles, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorenz\MC_Lorenz_Marginal_Likelihoods.png")

##-------------------------------------------------- Marginal Posterior for Lorenz Initial Conditions --------------------------------------------------------#
...
for param_idx in 1:3
    for (i, param_val) in enumerate(param_ranges[param_idx])
        ...
        logprior = logpdf(priors[1], u0_test[1]) + logpdf(priors[2], u0_test[2]) + logpdf(priors[3], u0_test[3])
        
        logpost_matrix[i, param_idx] = loglik + logprior  # No Jacobian adjustment for initial conditions
    end
end

pp1 = plot(param_ranges[1], logpost_matrix[:, 1], 
    linewidth=2, xlabel="x₀", ylabel="Log-Posterior", 
    title="x₀", grid=true, gridalpha=0.3, label="")
vline!(pp1, [u0[1]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(pp1, [state_est[1]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

pp2 = plot(param_ranges[2], logpost_matrix[:, 2], 
    linewidth=2, xlabel="y₀", ylabel="Log-Posterior", 
    title="y₀", grid=true, gridalpha=0.3, label="")
vline!(pp2, [u0[2]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(pp2, [state_est[2]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

pp3 = plot(param_ranges[3], logpost_matrix[:, 3], 
    linewidth=2, xlabel="z₀", ylabel="Log-Posterior", 
    title="z₀", grid=true, gridalpha=0.3, label="")
vline!(pp3, [u0[3]], linestyle=:dash, color=:red, label="True", linewidth=2)
vline!(pp3, [state_est[3]], linestyle=:dashdot, color=:blue, label="Estimated", linewidth=2)

p_posteriors = plot(pp1, pp2, pp3, layout=(1,3), size=(1300, 800), 
    plot_title="Marginal Log-Posterior - Lorenz Initial Conditions", left_margin=6mm, bottom_margin=5mm,
...
savefig(p_posteriors, raw"C:\Users\mussi\Documents\Manhattan\Leuven\MCQMC\Plots\is-mp-smmala_ad\Lorenz\MC_Lorenz_Marginal_Log-Posteriors.png")