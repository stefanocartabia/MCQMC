...
function simulate_system(f!::Function, u0, par, tspan::Tuple, dt)
    # build forward-sensitivity problem
    prob = ODEForwardSensitivityProblem(f!, u0, tspan, par)
    sol = solve(prob, Tsit5(); saveat=dt, reltol=1e-6, abstol=1e-8, save_everystep=false)
    x, dp = extract_local_sensitivities(sol)   # x: (n_states, n_times), dp: Vector of length n_params

    # stack dp into (n_states, n_times, n_params)
    n_params = length(dp)
    n_states, n_times = size(x)
    sens = Array{Float64}(undef, n_states, n_times, n_params)
    sens[:, :, 1:n_params] = dp[1:n_params]
...
function build_log_posterior_and_grad(f!, u0, obs, save_chol_cov::Chol_Save, tspan, dt, logprior_par::Function, d::Integer, K::Integer)
    ...
    function logpost_and_grad(theta::AbstractVector)
        par = exp.(theta)
        ...
        E = obs .- sim
        Y = similar(sim)
        ldiv!(Y, save_chol_cov.chol_cov.U, E)
        # dloglik/dsim = inv(Sigma) * (obs - sim) = U^{-T} * U^{-1} * (obs - sim)
        ldiv!(E, save_chol_cov.chol_cov.L, Y)  # E now contains inv(Sigma)*(obs-sim)
        
        # Contract with sensitivities: ∂loglik/∂p = sum over states and times of (∂loglik/∂sim * ∂sim/∂p)
        D = length(par)
        grad_p = zeros(D)
        for j in 1:D
            grad_p[j] = sum(E .* sens[:, :, j])
        end
        ...
        # Transform to theta space: ∂/∂theta = diag(par) * ∂/∂p + ∂logjac/∂theta
        grad_theta = grad_p .* par .+ ones(D)
...
function Tensor_Metric(f!, u0, theta::AbstractVector{<:Real}, tspan, dt, d::Integer, K::Integer, save_chol_cov::Chol_Save; λ::Float64 = 1e-1)
    D = length(theta)                       # numero di parametri
    U = save_chol_cov.chol_cov.U            # fattore triangolare superiore di Σ^{-1}

    p = collect(exp.(theta))                # parametri (tipo Array, compatibile)
    prob_sens = ODEForwardSensitivityProblem(f!, u0, tspan, p; sensealg = ForwardSensitivity())

    sol = solve(prob_sens, Tsit5(); saveat=dt, reltol=1e-6, abstol=1e-8, maxiters=10000, save_everystep=false)

    # estraggo valori della soluzione e sensitivities locali
    x, dp = extract_local_sensitivities(sol) 

    # costruiamo la Jacobiana J di dimensione (d*K) x D
    n_states, n_times = size(x)
    J = zeros(eltype(x), d*K, D)
    for j in 1:D
        Sj = dp[j]                     # n_states × n_times
        for t in 1:K
            r1 = (t-1)*d + 1; r2 = t*d
            J[r1:r2, j] = Sj[1:d, t]   # prendo solo le prime d variabili osservate
        end
    end

    # calcolo Fisher information: G = Σ_t J_t' Σ^{-1} J_t
    G = zeros(eltype(J), D, D)
    for t in 1:K
        r1 = (t-1)*d + 1; r2 = t*d
        Jt = @view J[r1:r2, :]         # d × D
        MJt = similar(Jt)
        mul!(MJt, U, Jt)               # MJt = U * Jt  (U' * U = Σ^{-1})
        mul!(G, transpose(MJt), MJt, 1.0, 1.0)  # G += (MJt)'*(MJt)
    end
...
function IS_MP_sMALA_LV(f!::Function, u0, obs, d::Integer, K::Integer, cov_mat, tspan, dt, logprior_par::Function, 
                        init_par::AbstractVector{<:Float64}; seq::AbstractMatrix{<:Float64},
                        N_prop::Integer, N_iter::Integer, step_size::Float64)
    ...
    log_post_i, grad = f_logpost_grad(init_par)
    ...
    for l in 1:N_iter
        ...
        proposals[1, :, l] = xI
        metric = Tensor_Metric(f!, u0, xI, tspan, dt, d, K, save)
        # Sample bridge variable z with MALA kernel K(z|xI)
        # Drift sm-MALA with ∇logπ(xI​)
        mu_x = xI + alpha * (metric.InvFisherInfo * grad_ad!(grad, f_logpost, xI))
        ...
        metric_z = Tensor_Metric(f!, u0, z, tspan, dt, d, K, save)
        mu_z     = z + alpha * (metric_z.InvFisherInfo * grad_ad!(grad, f_logpost, z))
        ...
        for i in 1:(N_prop + 1)
            ...
            # μ(yi): Langevin drift 
            metric_y = Tensor_Metric(f!, u0, yi, tspan, dt, d, K, save)
            mu_yi    = yi + alpha * (metric_y.InvFisherInfo * grad_ad!(grad, f_logpost, yi))
...
N_prop = 25 ; N_iter =  3000 
seq = rand(N_iter*(N_prop+1), 5)
a = [1.,1.,1.,1.]
init_par = log.(a)

############################################ Step size tuning  ################################################
tuning_time = @elapsed pre_run = IS_MP_sMALA_LV(lotka_volterra!, u0, obs_noisy, size(obs_noisy, 1), size(obs_noisy, 2), sigma_eta, tspan, dt, logprior_par,
...
mcqmc_time = @elapsed out = IS_MP_sMALA_LV(lotka_volterra!, u0, obs_noisy, size(obs_noisy, 1), size(obs_noisy, 2), sigma_eta, tspan, dt, logprior_par,
